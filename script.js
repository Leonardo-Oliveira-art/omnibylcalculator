...
// definição baseada no pH + alcalinidade
if(ph >= 7.8 && ph < 8.0){
    dosagem = (alcalinidade <= 100) ? 10 : 12;
}
else if(ph >= 8.0 && ph < 8.2){
    dosagem = (alcalinidade <= 100) ? 15 : 17;
}
else if(ph >= 8.2 && ph < 8.4){
    dosagem = (alcalinidade <= 100) ? 20 : 22;
}
else if(ph >= 8.4){
    dosagem = (alcalinidade <= 100) ? 25 : 27;
}

// cálculo final
let qtdRedutor = dosagem * volume;

resultado += `<p class='vermelho'>
pH Alto ⚠ Adicionar ${qtdRedutor.toFixed(0)} ml de redutor de pH
</p>`;
...
resultado += "<p class='vermelho'>Cloro Muito Alto ❌ Remover parte da água e medir os parâmetros</p>";
